...
def post_settings():
    buttons = [
        types.InlineKeyboardButton(text='Set text', callback_data='set_t'),
...
def start(message):
    ...
    if validation(data.u_id) != True:
        bot.send_message(data.u_id, 'Пожалуйста подпишитесь на канал @Ksarp_bots')
        return False;
...
@bot.callback_query_handler(func=lambda c: c.data == 'set_t')
def set_t(call):
    bot.answer_callback_query(call.id)
    call_id = call.from_user.id
    msg = bot.send_message(call_id, "Enter text:")
    bot.register_next_step_handler(msg, set_t2)
    

def set_t2(message):
    post_text.append(message.text)
    msg = bot.send_message(message.from_user.id, 'saved!', reply_markup=post_settings())
...
def show_m(call):
    ...
    bot.send_message(call_id, post_text[0], reply_markup=markup)
...
def send_to_everyone(call):
    ...
    for user in all_users():
        try:
            bot.send_message(user[0], post_text[0], reply_markup=markup)    
            good += 1
        except Exception as e:
            print(e)
            bad += 1
...
''')
            ...
            try:
                ...
                m2 = bot.reply_to(message, f"`{json2}`", reply_markup=keyboard)
